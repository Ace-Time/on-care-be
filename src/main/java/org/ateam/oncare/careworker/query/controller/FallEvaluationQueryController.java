...
public class FallEvaluationQueryController {
    ...
    // 1. 낙상위험도 평가 목록 조회 (요양보호사별)
    @GetMapping
    public ApiResponse<List<BasicEvaluationListDto>> getFallEvaluationList(
    ...
    // 2. 낙상위험도 평가 목록 조회 (수급자별)
    @GetMapping("/beneficiary/{beneficiaryId}")
    public ApiResponse<List<BasicEvaluationListDto>> getFallEvaluationListByBeneficiary(
    ...
    // 3. 낙상위험도 평가 상세 조회
    @GetMapping("/{evalId}")
    public ApiResponse<BasicEvaluationDetailDto> getFallEvaluationDetail(@PathVariable Long evalId) {
